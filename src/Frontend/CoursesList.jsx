...
const CardList = () => {
  ...
  const [modalVisible, setModalVisible] = useState(false); //CHECkLOGIN USING useState
  const [selectedCourse, setSelectedCourse] = useState(null); //COURSESELECTION USING useState
  const [currentUser, setCurrentUser] = useState(null); //USERCURRENT USING useState
  const [Checklogin, setChecklogin] = useState("NO"); //CHECkLOGIN USING useState
  const [Percentage, setPercentage] = useState(""); //PERCENTAGE USING useState
  const [applyBtnDisabled, setApplyBtnDisabled] = useState(true); //BUTTON USING useState
  const [PercentValidity, setPercentValidity] = useState(false); //PERCENTAGE'S VALIDITY  USING useState
  const [userRequests, setUserRequests] = useState([]);

  //-----LOCALSTROARGE SA GETITEM kAR k USER SHOW kRWANA
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      const fetchUserRequests = async () => {
        try {
          ...
          setUserRequests(userCourses); //--   SET USER REQUEST
        } catch (error) {
          console.error("Error fetching user requests:", error);
        }
      };

      fetchUserRequests(); //INVOkING
    }

    //----COURSE FETCHING USING API
    ...
    fetchCourses(); //INVOkING
  }, []);

  //SHOW MODAL IF ITS FALSE
  const showModal = (course) => {
    setSelectedCourse(course);
    setModalVisible(true);
  };

  //HIDING MODAL (CRITERIA)
  const hideModal = () => {
  ...
  //HASaPPLIED request = FETCHINGUSER
  // courseTitle ko selectedCourse  COMPARING
  const hasApplied = userRequests.some(
    (request) => request.courseTitle === selectedCourse?.title
  );

  const handleRequest = async (e) => {
    e.preventDefault(); // Default form submission stopiinGG

    if (hasApplied) {
      alert("You have already applied for this course.");
      return;
    }
    const criteriaRequired = parseInt(selectedCourse.criteria);  //string to int bczz ya DB ma string ma h
    //-----request jo user K ja rae
    const newRequest = {
                              ...
                              <div className="col-md-6">
                                ...
                                >
                                  *Percentage should be more{" "}
                                  ...
                                  {/* ---criteria means percentage jo mana set K h DB ma */}
                                </span>
                              </div>
                            </div>
                            <br />
...
const Card = ({ data, showModal }) => {
  ...
  const formatteddate = today.toISOString().split("T")[0];  //fomatted ma conversion
  const isTestToday = data.date === formatteddate;
  const isExpired = data.date < formatteddate;
